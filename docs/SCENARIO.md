...
### 충돌 #1 — 자명한 충돌 (add/add)
- 정인호(#6)와 이교원(#7)이 **같은 시점**에 `main`(아직 `src/utils.py` 없음)에서 각자 브랜치를 생성합니다.
- 두 사람 모두 `src/utils.py`를 새로 만들면서 각자 함수를 추가합니다.
- 정인호 PR을 먼저 머지 → 이교원 PR을 머지하려 하면, 두 브랜치가 같은 경로에 서로 다른 내용의 파일을 각각 "새로 추가"한 상태라 `CONFLICT (add/add): Merge conflict in src/utils.py`가 발생합니다. (기존 파일의 같은 줄을 고치는 modify/modify 충돌이 아니라, 파일이 없던 상태에서 둘 다 새로 만든 add/add 충돌입니다 — 충돌 마커 안에 두 사람의 코드가 나란히 보이고 둘 다 살리면 되므로, 결과 화면과 해결 난이도는 동일하게 "자명"합니다.)
- 이교원이 로컬에서 `git fetch origin && git merge origin/main` 후 충돌 마커를 직접 해결하고 push.

### 충돌 #2 — 비자명한 충돌 (파일 이동 vs 내용 수정)
- 박기태(#9)가 `git mv src/utils.py src/string_utils.py`로 파일명을 바꾸는 브랜치를 작업하는 동안,
- 같은 시점에 다른 팀원(정인호 또는 이교원)이 `src/utils.py`의 **같은 줄**(예: 상단 docstring 한 줄)을 수정하는 브랜치를 먼저 머지합니다.
- **박기태의 rename 브랜치도 그 같은 줄을 함께 수정해야 합니다.** 그렇지 않으면 git의 기본 병합 전략(`ort`)이 파일 이동을 자동으로 감지해서, 겹치지 않는 변경(이동 vs 다른 위치 수정)은 충돌 없이 조용히 자동 병합해버립니다. 겹치는 줄이 있어야만 실제 `CONFLICT (content): Merge conflict in src/string_utils.py`가 발생합니다.
- `git status`를 보면 `src/utils.py`는 삭제(`D`)로, `src/string_utils.py`는 충돌(`UU`)로 동시에 표시되어 처음 보면 혼란스럽습니다. 충돌이 원래 작업하던 경로가 아니라 **상대방이 옮긴 새 경로**에서 발생한다는 점, 그리고 겹치지 않으면 애초에 충돌조차 안 난다는 점이 이 충돌을 "비자명"하게 만드는 핵심입니다.
- 박기태가 나중에 `git fetch origin && git merge origin/main` 후, 이동된 파일(`string_utils.py`) 안의 충돌 마커를 직접 해결하고 push.
